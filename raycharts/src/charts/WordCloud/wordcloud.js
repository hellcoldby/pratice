    ...
    var WordCloud = function WordCloud(elements, options) {
        ...
        var drawText = function drawText(
            ...
            info.fillTextWidth, info.fillTextHeight) */
                    ...
                    ctx.restore();
                    //增加监听绘制调用
                    if (settings.onCanvasDraw) {
                        settings.onCanvasDraw({
                            scale: [1 / mu, 1 / mu],
                            position: [(gx + info.gw / 2) * g * mu, (gy + info.gh / 2) * g * mu],
                            x: info.fillTextOffsetX * mu,
                            y: (info.fillTextOffsetY + fontSize * 0.5) * mu,
                            rotation: rotateDeg,
                            word,
                            color,
                            font: fontWeight + ' ' + (fontSize * mu).toString(10) + 'px ' + settings.fontFamily,
                            extraData: extraDataArray[0],
                        });
                    }
                } else {
