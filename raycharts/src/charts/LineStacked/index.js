...
function LineStacked(config) {
    ...
    const shape_animList = animType_list.getAnim('shape');

    // 判断是否有生长动画
    let shape_isGrow = false;
    const grow_index = shape_animList.indexOf('grow');
    if (grow_index !== -1) {
        shape_animList.splice(grow_index, 1);
        shape_isGrow = true;
    }
    ...
    positionList.reduce((prev, { position, cid }, i, pArr) => {
        ...
        seriesList.reduce(
            ({ upPoint, downPoint }, { sid, name }, j) => {
                ...
                const basePoint = itemHeight >= 0 ? upPoint : downPoint;
                ...
                chartGroup.add(pointSet);
                return {
                    upPoint: itemHeight >= 0 ? targetPoint : upPoint,
                    downPoint: itemHeight >= 0 ? downPoint : targetPoint,
                };
            },
            {
                upPoint: position,
